---
name: codex-ask
description: Ask Codex for a grounded second opinion on any question - advisory, not gating. Enter when the user wants a second opinion, sanity check, or "ask Codex/GPT" on a design, hypothesis, debugging theory, or trade-off.
...
- Model/effort/tier come from `codex-plan-review/scripts/_common.sh` (non-implement flows get the review-class model by default: gpt-5.6-sol @ xhigh, priority tier); override per run via `CODEX_MODEL` / `CODEX_EFFORT` / `CODEX_SERVICE_TIER`. Full flag reference: SETUP.md §6.
