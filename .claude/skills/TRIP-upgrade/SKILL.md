...
**Preserve `state/`**: it holds live thread ids and the cumulative `usage.ndjson`
telemetry — a naive `rm -rf` destroys both. Set it aside, replace, then merge it back
(the merge keeps the new skill's `.gitignore` while restoring the project's state files):

```bash
if [ -d .claude/skills/<skill>/state ]; then mv .claude/skills/<skill>/state /tmp/_upg-state; fi
rm -rf .claude/skills/<skill>/
cp -r <staging-path>/<skill>/ .claude/skills/<skill>/
if [ -d /tmp/_upg-state ]; then
    mkdir -p .claude/skills/<skill>/state
    cp -r /tmp/_upg-state/. .claude/skills/<skill>/state/
    rm -rf /tmp/_upg-state
fi
```
...
Run the end-to-end self-test — three Luna/low Codex calls, ~90s:

```bash
bash .claude/skills/codex-plan-review/scripts/smoke.sh
```

Any FAIL line blocks the upgrade from being declared complete.
...
`AskUserQuestion`: "The new workflow needs lint/typecheck/test commands for the testing gate that precedes code review. What are the commands for this project?"
...
- **New workflow features get project context.** When the code-review gate is new, the test commands still need to be filled from the project's existing test setup.
